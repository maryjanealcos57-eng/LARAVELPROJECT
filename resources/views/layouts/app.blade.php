...
<head>
    ...
    <title>Alcos Task Manager</title>
    ...
    <style>
        body {
            ...
            background: linear-gradient(180deg, #bfe7ff 0%, #dff3ff 22%, #eff9ff 45%, #f7fcff 70%, #ffffff 100%);
        ...
        /* Soft drifting cloud shapes to match the sky-blue theme */
        .cloud-shape {
            position: fixed;
            z-index: 0;
            pointer-events: none;
            opacity: 0.55;
            filter: drop-shadow(0 10px 20px rgba(2, 132, 199, 0.08));
            animation: drift 40s ease-in-out infinite;
        }
        .cloud-shape.slow { animation-duration: 55s; opacity: 0.4; }
        .cloud-shape.reverse { animation-direction: reverse; }
        @keyframes drift {
        ...
        /* --- Hover dominance & balance ---
           The hovered card rises above its siblings (dominance) while every
           other card recedes by the same amount (balance), keeping the
           grid symmetric instead of chaotic. */
        #taskGrid .task-card {
        ...
        #taskGrid .task-card:hover {
            ...
            opacity: 1;
            filter: saturate(1.1);
            box-shadow: 0 20px 34px -12px rgba(2, 132, 199, 0.35);
            z-index: 10;
            position: relative;
        }
...
<body class="text-ink min-h-screen flex flex-col md:flex-row relative overflow-x-hidden">
    ...
    <!-- Decorative background: soft glow blobs + drifting clouds -->
    ...
    <svg class="cloud-shape w-40 top-10 left-[8%]" viewBox="0 0 120 60" fill="white" xmlns="http://www.w3.org/2000/svg">
        <ellipse cx="30" cy="38" rx="26" ry="18"/><ellipse cx="58" cy="28" rx="30" ry="22"/><ellipse cx="88" cy="38" rx="24" ry="16"/>
    </svg>
    <svg class="cloud-shape slow reverse w-56 top-[22%] right-[6%]" viewBox="0 0 120 60" fill="white" xmlns="http://www.w3.org/2000/svg">
        <ellipse cx="30" cy="38" rx="26" ry="18"/><ellipse cx="58" cy="28" rx="30" ry="22"/><ellipse cx="88" cy="38" rx="24" ry="16"/>
    </svg>
    <svg class="cloud-shape w-32 top-[55%] left-[18%]" viewBox="0 0 120 60" fill="white" xmlns="http://www.w3.org/2000/svg">
        <ellipse cx="30" cy="38" rx="26" ry="18"/><ellipse cx="58" cy="28" rx="30" ry="22"/><ellipse cx="88" cy="38" rx="24" ry="16"/>
    </svg>
    <svg class="cloud-shape slow w-44 bottom-[10%] right-[16%]" viewBox="0 0 120 60" fill="white" xmlns="http://www.w3.org/2000/svg">
        <ellipse cx="30" cy="38" rx="26" ry="18"/><ellipse cx="58" cy="28" rx="30" ry="22"/><ellipse cx="88" cy="38" rx="24" ry="16"/>
    </svg>
    ...
    <aside class="relative z-10 w-full md:w-64 bg-white/70 backdrop-blur-md border-b md:border-b-0 md:border-r border-sky/20 flex flex-col justify-between shrink-0 md:h-screen md:sticky md:top-0">
        <div>
            ...
            <div class="h-24 flex items-center px-6 border-b border-sky/20 gap-3">
                ...
                <div>
                    <h1 class="font-display font-extrabold text-2xl leading-tight text-skydeep tracking-tight">Alcos</h1>
            ...
            <nav class="p-4 space-y-2">
                ...
                <a href="#" onclick="filterTasks('pending')" id="nav-pending" class="flex items-center gap-3 px-5 py-3 rounded-full text-sm font-medium text-[#5a8aa3] hover:bg-skytint transition-all">
                    <span class="w-6 h-6 rounded-full bg-amber-100 text-amber-500 flex items-center justify-center"><i class="fa-solid fa-clock text-[11px]"></i></span> Pending <span id="count-pending" class="ml-auto w-6 h-6 rounded-full bg-skytint flex items-center justify-center text-xs font-semibold text-skydeep">0</span>
                </a>
                <a href="#" onclick="filterTasks('completed')" id="nav-completed" class="flex items-center gap-3 px-5 py-3 rounded-full text-sm font-medium text-[#5a8aa3] hover:bg-skytint transition-all">
                    <span class="w-6 h-6 rounded-full bg-emerald-100 text-emerald-500 flex items-center justify-center"><i class="fa-solid fa-circle-check text-[11px]"></i></span> Completed <span id="count-completed" class="ml-auto w-6 h-6 rounded-full bg-skytint flex items-center justify-center text-xs font-semibold text-skydeep">0</span>
                </a>
            </nav>
        </div>
        ...
        <div class="p-4 border-t border-sky/20">
            <div class="flex items-center gap-3 p-3 rounded-full bg-skytint/60">
                ...
                <div class="overflow-hidden">
                    <p class="text-sm font-semibold text-ink truncate">Alcos</p>
                    <p class="text-xs text-[#5a8aa3] truncate">Task Manager</p>
                </div>
            </div>
        </div>
    </aside>
    ...
    <main class="relative z-10 flex-1 flex flex-col min-w-0">
        ...
        <header class="h-24 bg-white/50 backdrop-blur-md border-b border-sky/20 px-6 md:px-10 flex items-center justify-between gap-4 sticky top-0 z-10">
            ...
            <div class="flex items-center gap-3">
                <select id="priorityFilter" onchange="applyFilters()" class="border-2 border-skytint bg-white rounded-full px-4 py-2.5 text-xs text-[#5a8aa3] focus:outline-none focus:border-sky transition-colors hidden sm:block">
        ...
        <div class="p-6 md:p-10 space-y-8 flex-1 max-w-7xl w-full mx-auto">
            ...
            <!-- Stats Bar (circular icon badges) -->
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-5">
                <div class="bg-white/80 border-2 border-skytint rounded-3xl p-5 flex items-center gap-4 shadow-sm">
                    ...
                    <div>
                        <p class="text-xs uppercase tracking-wide text-[#5a8aa3] font-semibold mb-0.5">Total</p>
                        <h3 id="statTotal" class="text-2xl font-bold text-ink">0</h3>
                    </div>
                </div>
                <div class="bg-white/80 border-2 border-skytint rounded-3xl p-5 flex items-center gap-4 shadow-sm">
                    <div class="w-14 h-14 rounded-full bg-gradient-to-br from-amber-300 to-amber-400 text-white flex items-center justify-center text-lg shrink-0">
                        <i class="fa-solid fa-hourglass-half"></i>
                    </div>
                    <div>
                        <p class="text-xs uppercase tracking-wide text-[#5a8aa3] font-semibold mb-0.5">Pending</p>
                        <h3 id="statPending" class="text-2xl font-bold text-ink">0</h3>
                    </div>
                </div>
                <div class="bg-white/80 border-2 border-skytint rounded-3xl p-5 flex items-center gap-4 shadow-sm">
                    <div class="w-14 h-14 rounded-full bg-gradient-to-br from-emerald-300 to-emerald-400 text-white flex items-center justify-center text-lg shrink-0">
                        <i class="fa-solid fa-circle-check"></i>
                    </div>
                    <div>
                        <p class="text-xs uppercase tracking-wide text-[#5a8aa3] font-semibold mb-0.5">Completed</p>
            ...
            <div>
                ...
                <div id="emptyState" class="hidden py-24 text-center border-2 border-dashed border-skytint rounded-3xl bg-white/60">
                    ...
                    <p class="text-xs text-[#5a8aa3] mt-1">Add your first task using the button above.</p>
                </div>
    ...
    <div id="taskModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-[#0c3a52]/30 backdrop-blur-sm hidden opacity-0 transition-opacity duration-200">
        <div class="bg-white border-2 border-skytint w-full max-w-lg rounded-[2rem] overflow-hidden transform scale-95 transition-transform duration-200 shadow-2xl shadow-sky/20" id="modalCard">
            ...
            <form id="taskForm" onsubmit="handleFormSubmit(event)" class="p-6 space-y-4">
                ...
                <div>
                    <label class="block text-xs font-semibold text-[#5a8aa3] mb-1.5">Task Title *</label>
                    <input type="text" id="taskTitle" required placeholder="e.g., Redesign landing page header..." class="w-full border-2 border-skytint rounded-full px-5 py-2.5 text-sm placeholder-sky/50 focus:outline-none focus:border-sky transition-colors">
                </div>
                <div>
                    <label class="block text-xs font-semibold text-[#5a8aa3] mb-1.5">Description</label>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        <label class="block text-xs font-semibold text-[#5a8aa3] mb-1.5">Priority Level</label>
                    ...
                    <div>
                        <label class="block text-xs font-semibold text-[#5a8aa3] mb-1.5">Due Date *</label>
                ...
                <div class="pt-4 border-t border-skytint flex items-center justify-end gap-3">
                    <button type="button" onclick="closeModal()" class="px-5 py-2.5 rounded-full text-[#5a8aa3] hover:text-skydeep font-medium text-sm transition-colors">Cancel</button>
    ...
    <script>
        ...
        function filterTasks(filter) {
            ...
            const inactiveClass = "flex items-center gap-3 px-5 py-3 rounded-full text-sm font-medium text-[#5a8aa3] hover:bg-skytint transition-all";
        ...
        // Render the task grid with visible round action buttons on each card
        function renderTaskGrid() {
            ...
            filtered.forEach(task => {
                ...
                if (task.priority === 'High') { priorityColor = 'text-rose-500'; priorityBg = 'bg-rose-100'; }
                if (task.priority === 'Medium') { priorityColor = 'text-amber-500'; priorityBg = 'bg-amber-100'; }
                ...
                const statusBadge = isCompleted
                    ...
                    : `<span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold bg-amber-100 text-amber-600"><i class="fa-solid fa-clock text-[10px]"></i> Pending</span>`;
                ...
                card.innerHTML = `
                    <div>
                        <div class="flex items-start justify-between gap-3 mb-3">
                            <span class="w-8 h-8 rounded-full ${priorityBg} ${priorityColor} flex items-center justify-center text-xs"><i class="fa-solid fa-star"></i></span>
                            ${statusBadge}
                        </div>
                        <h4 class="font-display font-semibold text-base text-ink mb-1.5 ${isCompleted ? 'line-through text-[#9cc3d6]' : ''}">${escapeHtml(task.title)}</h4>
                        <p class="text-xs text-[#5a8aa3] line-clamp-2">${escapeHtml(task.description || 'No description provided.')}</p>
                    </div>
                    <div>
                        ...
                        <div class="flex items-center justify-center gap-2">
                            ...
                            <button onclick="deleteTask('${task.id}')" title="Delete" class="circle-btn w-10 h-10 rounded-full border-2 border-skytint text-[#6aa5c2] hover:bg-rose-50 hover:text-rose-500 flex items-center justify-center">
